...
class DatatypePredicateAtomHandler implements XmlTypeHandler {
	...
	public Object importObject(Object obj) throws R2MLException {
		...
		if ((Number.class.isAssignableFrom(arg0.getType()) && Number.class
			...
			c.setSlotNames(R2MLDriver.get().getNameMapper().getSlotNames(atom.getDatatypePredicateID()));
			fact.setId(c.getName());
			fact.setPredicate(c);
		} else if ((symbol.equals("==") || symbol.equals("!=")) && 
				((arg0.getType() == Boolean.class && arg1.getType() == Boolean.class)
						|| (arg0.getType() == String.class && arg1.getType() == String.class))) {
			c.setNegated(R2MLUtil.isNegated(atom));
			c.setTypes(new Class[] { arg1.getType(), arg0.getType() });
			c.setSlotNames(R2MLDriver.get().getNameMapper().getSlotNames(atom.getDatatypePredicateID()));
			fact.setId(c.getName());
	 ...
	 */
	private void buildAsJPredicate(DatatypePredicateAtom atom, Fact fact, String symbol) throws R2MLException {
		...
		if (atom.getDataArguments().getDataTerm().get(0).getDeclaredType() == TypedLiteral.class) {
			aft = (AttributeFunctionTerm)(atom.getDataArguments().getDataTerm().get(1).getValue());
			tl = (TypedLiteral)(atom.getDataArguments().getDataTerm().get(0).getValue());
			objT = aft.getContextArgument().getObjectTerm().getValue();
		} else {
			aft = (AttributeFunctionTerm)(atom.getDataArguments().getDataTerm().get(0).getValue());
			tl = (TypedLiteral)(atom.getDataArguments().getDataTerm().get(1).getValue());
			objT = aft.getContextArgument().getObjectTerm().getValue();
		}
		// value of the typed literal
		boolean tlv = new Boolean(tl.getLexicalValue());
		XmlTypeHandler handler = R2MLDriver.get().getHandlerByXmlType(aft.getContextArgument().getObjectTerm().getDeclaredType());
		...
		fact.setPredicate(AttributionAtomHandler.buildJPredciate(domain, aft.getAttributeID().getLocalPart(), isNegated(atom, symbol, tlv), slotNames));
		fact.setTerms(new Term[]{domain});
	}

	private boolean isNegated(DatatypePredicateAtom atom, String symbol, boolean tlv) {
