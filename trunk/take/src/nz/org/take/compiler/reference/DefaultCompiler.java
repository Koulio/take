...
public class DefaultCompiler extends CompilerUtils  implements Compiler {
	...
	private Map<String,SimplePredicate> externalFactStores = null;
	...
	public void compileInterface (KnowledgeBase kb) throws CompilerException  {
		...
		for (Map.Entry<String,SimplePredicate>entry:this.getExternalFactStores().entrySet()) {
			String pClassName = "ExternalFactStore_"+getClassName(entry.getValue());
			String fullClassName = packageName + "." + pClassName;
			PrintWriter out = new PrintWriter(location.getSrcOut(fullClassName));
			createExternalFactStoreType(out, pClassName, packageName, entry.getKey(),entry.getValue());
			out.close();
			endorseClazz(location, fullClassName);
		}
	 ...
	 * @param pck the package name
	 * @param fsId teh fact store id
	 * @param p the predicate
	 */
	private void createExternalFactStoreType(PrintWriter out, String clazz, String pck,String fsId,Predicate p) throws CompilerException {
		Slot[] slots = buildSlots(p);
		out.print("package ");
		out.print(pck);
		out.println(";");
   ...
   /**
	...
	private  Map<String,SimplePredicate> getExternalFactStores() {
		if (this.externalFactStores==null) {
			externalFactStores = new HashMap<String,SimplePredicate>();
			for (KnowledgeElement e:kb.getElements()) {
				if (e instanceof ExternalFactStore) {
					ExternalFactStore fs = (ExternalFactStore)e;
					externalFactStores.put(fs.getId(),fs.getPredicate());
				}
			}
		}
