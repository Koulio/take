 ...
 * This is usually done dynamically at runtime. 
...
public class GenerateClasses {
	...
	public static void main(String[] args) throws Exception {
		...
		BasicConfigurator.configure();			
		// generate kb
		InputStream script = GenerateClasses.class.getResourceAsStream("/example/nz/org/take/compiler/example1/crm-example.take");
		ScriptKnowledgeSource ksource = new ScriptKnowledgeSource(script);
		compiler.setInterfaceNames("example.nz.org.take.compiler.example1.spec.DiscountPolicy");
		compiler.setLocation(location);
		compiler.setPackageName("example.nz.org.take.compiler.example1.impl");
		compiler.setClassName("DiscountPolicy");
		compiler.setGenerateDataClassesForQueryPredicates(false);
		compiler.setImportStatements("example.nz.org.take.compiler.example1.spec.*");
